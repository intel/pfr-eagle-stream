...
void SYSTEM_MOCK::force_mock_entropy_error()
{
    alt_u32* cry_ptr = (alt_u32*) __IO_CALC_ADDRESS_NATIVE_ALT_U32(U_CRYPTO_AVMM_BRIDGE_BASE, 0);
    set_mem_word(cry_ptr, CRYPTO_CSR_ENTROPY_SOURCE_HEALTH_TEST_ERROR_MSK, false);
}
...
void SYSTEM_MOCK::reset_after_cpld_reconfiguration()
{
    ...
    // Reset the mailbox and the settings
...
void SYSTEM_MOCK::stage_next_spdm_message()
{
    //
    alt_u32* mb_ptr = (alt_u32*) __IO_CALC_ADDRESS_NATIVE(U_MAILBOX_AVMM_BRIDGE_BASE, 0);
    set_mem_word(mb_ptr + MB_MCTP_PACKET_WRITE_RXFIFO, 220, false);
}
