...
alt_u32 SPI_CONTROL_MOCK::get_mem_word(void* addr)
{
    ...
    else if (m_spi_master_csr.is_addr_in_range(addr))
    {
        ...
        alt_u32 spi_command = m_spi_master_csr.get_mem_word(SPI_CONTROL_1_CSR_CS_FLASH_COMMAND_SETTING_ADDR) & 0xFFFF;

        if (csr_offset == SPI_CONTROL_1_CSR_CS_FLASH_COMMAND_READ_DATA0_OFST)
        {
            if (spi_command == (0x1800 | SPI_CMD_READ_STATUS_REG))
            {
                alt_u32 reg_val = m_spi_master_csr.get_mem_word(addr);
                //m_spi_master_csr.set_mem_word(addr, 0x00);
                return reg_val;
            }
        }
    }
