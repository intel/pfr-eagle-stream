...
TEST_F(PFRAntiRollbackWithSVNTest, test_sanity)
{
    // Use BMC staging are for this test
    switch_spi_flash(SPI_FLASH_BMC);
    alt_u32 signed_payload_addr =  get_ufm_pfr_data()->bmc_staging_region;

    /*
     * Step 1. All capsules should pass authentication before
     * Using check_capsule_before_update function because SVN is only checked before update
     */
    //Load BMC capsule and authenticate
    SYSTEM_MOCK::get()->load_to_flash(
            SPI_FLASH_BMC,
            SIGNED_CAPSULE_BMC_WITH_SVN2_FILE,
            SIGNED_CAPSULE_BMC_WITH_SVN2_FILE_SIZE,
            signed_payload_addr);
    EXPECT_TRUE(check_capsule_before_update(signed_payload_addr, MB_UPDATE_INTENT_BMC_RECOVERY_MASK, 0));

    //Load PCH capsule and authenticate
    SYSTEM_MOCK::get()->load_to_flash(
            SPI_FLASH_BMC,
            SIGNED_CAPSULE_PCH_FILE,
            SIGNED_CAPSULE_PCH_FILE_SIZE,
            signed_payload_addr);
    EXPECT_TRUE(check_capsule_before_update(signed_payload_addr, MB_UPDATE_INTENT_PCH_RECOVERY_MASK, 0));

    //Load CPLD capsule and authenticate
    SYSTEM_MOCK::get()->load_to_flash(
            SPI_FLASH_BMC,
            SIGNED_CAPSULE_CPLD_WITH_SVN64_FILE,
            SIGNED_CAPSULE_CPLD_WITH_SVN64_FILE_SIZE,
            signed_payload_addr);
    EXPECT_TRUE(check_capsule_before_update(signed_payload_addr, MB_UPDATE_INTENT_CPLD_RECOVERY_MASK, 0));

    /*
     * Step 2. Upgrade to SVN 3 for BMC FW
     * BMC capsule with SVN 2 should fail the check, while others still pass
     */
    ...
    SYSTEM_MOCK::get()->load_to_flash(
            ...
            SIGNED_CAPSULE_BMC_WITH_SVN2_FILE_SIZE,
            signed_payload_addr);
    EXPECT_FALSE(check_capsule_before_update(signed_payload_addr, MB_UPDATE_INTENT_BMC_RECOVERY_MASK, 0));
    ...
    /*
     * Step 3. Upgrade to SVN PCH_UPDATE_CAPSULE_PFM_SVN for PCH FW
     * PCH capsule with SVN PCH_UPDATE_CAPSULE_PFM_SVN should fail the check.
     * There's no new change to BMC SVN, so the BMC capsule should still fail the check.
     * CPLD capsule should pass the check
     */
    write_ufm_svn(UFM_SVN_POLICY_PCH, PCH_UPDATE_CAPSULE_PFM_SVN + 1);
    ...
    SYSTEM_MOCK::get()->load_to_flash(
            ...
            SIGNED_CAPSULE_PCH_FILE_SIZE,
            signed_payload_addr);
    EXPECT_FALSE(check_capsule_before_update(signed_payload_addr, MB_UPDATE_INTENT_PCH_RECOVERY_MASK, 0));
    ...
    /*
     * Step 3. Upgrade to SVN 2 for CPLD image
     * All capsules should now fail the SVN check.
     */
    write_ufm_svn(UFM_SVN_POLICY_CPLD, 2);
    ...
    SYSTEM_MOCK::get()->load_to_flash(
            ...
            signed_payload_addr);
    EXPECT_FALSE(check_capsule_before_update(signed_payload_addr, MB_UPDATE_INTENT_CPLD_RECOVERY_MASK, 0));
}
...
TEST_F(PFRAntiRollbackWithSVNTest, test_active_fw_update_with_banned_svn)
{
    ...
    write_ufm_svn(UFM_SVN_POLICY_PCH, 8);
...
TEST_F(PFRAntiRollbackWithSVNTest, test_bmc_recovery_update_with_lower_svn)
{
    ...
    // Send the update intent
    write_to_mailbox(MB_BMC_UPDATE_INTENT_PART1, MB_UPDATE_INTENT_BMC_RECOVERY_MASK);
...
TEST_F(PFRAntiRollbackWithSVNTest, test_bump_svn_with_recovery_cpld_update)
{
    ...
    // Send the active update intent
    ut_send_in_update_intent_once_upon_entry_to_t0(MB_BMC_UPDATE_INTENT_PART1, MB_UPDATE_INTENT_CPLD_RECOVERY_MASK);
    ut_run_main(CPLD_CFM1, true);
