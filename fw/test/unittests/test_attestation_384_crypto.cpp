...
TEST_F(AttestationFlowTest, test_cfm_hash_change_after_cpld_active_update_384)
{
    /*
     * Test Preparation
     */
    ...
    // Track the cfm1 hash
    alt_u32 cfm1_hash_before_update[SHA384_LENGTH/4] = {0};
...
TEST_F(AttestationFlowTest, test_pub_key_0_change_after_cpld_active_update_384)
{
    /*
     * Test Preparation
     */
    ...
    alt_u32 pubkey_0_before_update[SHA384_LENGTH/2] = {0};
...
TEST_F(AttestationFlowTest, test_priv_key_1_change_after_cpld_active_update_384)
{
    /*
     * Test Preparation
     */
    ...
    alt_u32 privkey_1_before_update[SHA384_LENGTH/4] = {0};
...
TEST_F(AttestationFlowTest, test_cdi_1_change_after_cpld_active_update_384)
{
    /*
     * Test Preparation
     */
    ...
    // Track the cdi 1
    alt_u32 cdi_1_before_update[SHA384_LENGTH/4] = {0};
...
TEST_F(AttestationFlowTest, test_pubkey_1_change_after_cpld_active_update_384)
{
    /*
     * Test Preparation
     */
    ...
    // Track the pubkey 1
    alt_u32 pubkey_1_before_update[SHA384_LENGTH/2] = {0};
 ...
 * @brief Execute the recovery flow in the manner that user would normally do.
 */
TEST_F(AttestationFlowTest, test_key_generation)
{
    ...
    alt_u32_memcpy(uds_cfm0_hash_ptr, cfm0_uds, 12*4);

    uds_cfm0_hash_ptr = &uds_cfm0_hash[SHA384_LENGTH / 4];

    // Group cfm0 hash and uds
    calculate_and_save_sha(uds_cfm0_hash_ptr, UFM_CPLD_ROM_IMAGE_OFFSET, get_ufm_ptr_with_offset(UFM_CPLD_ROM_IMAGE_OFFSET),
                           UFM_CPLD_ROM_IMAGE_LENGTH, CRYPTO_384_MODE, DISENGAGE_DMA);

    // CDIO
    uds_cfm0_hash_ptr = &uds_cfm0_hash[0];

    // Create cdio
    calculate_and_save_sha(uds_cfm0_hash_ptr, 0, uds_cfm0_hash_ptr,
                           96, CRYPTO_384_MODE, DISENGAGE_DMA);

    alt_u32 priv_key0[SHA384_LENGTH] = {0};
    // Create private key 0
    calculate_and_save_sha((alt_u32*)priv_key0, 0, uds_cfm0_hash_ptr,
                           48, CRYPTO_384_MODE, DISENGAGE_DMA);

    alt_u32 pub_key[96/4] = {0};
    alt_u32* pb_key_ptr = pub_key;
    alt_u32* stored_pubkey = get_ufm_ptr_with_offset(UFM_CPLD_PUBLIC_KEY_0);

    generate_pubkey((alt_u32*)pb_key_ptr, pb_key_ptr + 12, (alt_u32*)priv_key0, CRYPTO_384_MODE);

    for (alt_u32 i = 0; i < 96/4; i++)
    {
        EXPECT_EQ(pub_key[i], stored_pubkey[i]);
    }

    alt_u32 cdi0_cfm1[SHA384_LENGTH/2]= {0};
    alt_u32_memcpy((alt_u32*)&cdi0_cfm1[SHA384_LENGTH/4], uds_cfm0_hash_ptr, 12*4);
    uds_cfm0_hash_ptr = &cdi0_cfm1[0];

    // Group cfm1 hash and cdio
    calculate_and_save_sha(uds_cfm0_hash_ptr, UFM_CPLD_ACTIVE_IMAGE_OFFSET, get_ufm_ptr_with_offset(UFM_CPLD_ACTIVE_IMAGE_OFFSET),
    		UFM_CPLD_ACTIVE_IMAGE_LENGTH, CRYPTO_384_MODE, DISENGAGE_DMA);

    alt_u32 cdi1[SHA384_LENGTH/4] = {0};

    // Create cdi1
    calculate_and_save_sha((alt_u32*)cdi1, 0, uds_cfm0_hash_ptr,
                           96, CRYPTO_384_MODE, DISENGAGE_DMA);

    alt_u32* stored_cdi1 = get_ufm_ptr_with_offset(UFM_CPLD_CDI1);

    for (alt_u32 i = 0; i < 48/4; i++)
    {
        EXPECT_EQ(cdi1[i], stored_cdi1[i]);
    }
}
