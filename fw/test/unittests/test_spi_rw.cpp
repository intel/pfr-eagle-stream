...
TEST_F(SPIFlashRWTest, test_memcpy_signed_payload_with_pch_pfm)
{
    ...
    SYSTEM_MOCK::get()->load_to_flash(
        SPI_FLASH_BMC, SIGNED_PFM_PCH_FILE, SIGNED_PFM_PCH_FILE_SIZE, signed_payload_addr);
    ...
    bmc_flash_ptr[(signed_payload_addr + SIGNED_PFM_PCH_FILE_SIZE - 4) >> 2] = 0xdecaf369;
    bmc_flash_ptr[(signed_payload_addr + SIGNED_PFM_PCH_FILE_SIZE) >> 2] = 0xdeadbeef;
    ...
    for (alt_u32 word_i = 0; word_i < ((SIGNED_PFM_PCH_FILE_SIZE - 4) >> 2); word_i++)
    {
    ...
    EXPECT_EQ(bmc_flash_ptr[0], alt_u32(BLOCK0_MAGIC));
    EXPECT_EQ(bmc_flash_ptr[(dest_addr + SIGNED_PFM_PCH_FILE_SIZE - 4) >> 2], alt_u32(0xdecaf369));
    EXPECT_NE(bmc_flash_ptr[(dest_addr + SIGNED_PFM_PCH_FILE_SIZE) >> 2], alt_u32(0xdeadbeef));

    // as sanity, make sure the source location is not change in the process.
    EXPECT_EQ(bmc_flash_ptr[(signed_payload_addr + SIGNED_PFM_PCH_FILE_SIZE - 4) >> 2],
              alt_u32(0xdecaf369));
    EXPECT_EQ(bmc_flash_ptr[(signed_payload_addr + SIGNED_PFM_PCH_FILE_SIZE) >> 2],
              alt_u32(0xdeadbeef));

    // Calculate expected number of erases
    // Since the destination address doesn't align with 64KB boundary, 64KB erase won't be used.
    alt_u32 expected_num_4kb_erases = SIGNED_PFM_PCH_FILE_SIZE / 0x1000;
    if (SIGNED_PFM_PCH_FILE_SIZE % 0x1000)
    {
        // if the capsule is not 4KB aligned, an additional erase would be executed.
        expected_num_4kb_erases++;
    }
