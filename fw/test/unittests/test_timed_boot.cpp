...
TEST_F(PFRTimedBootTest, test_timed_boot_flow_twice_dc_cycle_phase)
{
    // Timed boot is only enabled in provisioned state.
    EXPECT_TRUE(is_ufm_provisioned());
    ...
    // Simulate DC cycle
    // Nios mock wrapper will force pltrst
    ...
    write_to_mailbox(MB_PLATFORM_STATE, PLATFORM_STATE_T0_BMC_BOOTED);
