...
/**
 * @brief Test PIT L2 feature in normal use case.
 */
TEST_F(PFRProtectInTransitTest, test_pit_l2_happy_path)
{
    ...
    ASSERT_DURATION_LE(GTEST_TIMEOUT_LONG, EXPECT_ANY_THROW({ pfr_main(); }));
    ...
    alt_u8 exepected_pch_fw_hash[FULL_PFR_IMAGE_PCH_FILE_HASH_SIZE] = FULL_PFR_IMAGE_PCH_FILE_HASH;
    alt_u8* ufm_pit_bmc_fw_hash = (alt_u8*) get_ufm_pfr_data()->pit_bmc_fw_hash;
    alt_u8 exepected_bmc_fw_hash[FULL_PFR_IMAGE_BMC_FILE_HASH_SIZE] = FULL_PFR_IMAGE_BMC_FILE_HASH;

    alt_u32 sha_length = SHA256_LENGTH;
#ifdef GTEST_USE_CRYPTO_384
    sha_length = SHA384_LENGTH;
#endif
    for (alt_u32 byte_i = 0; byte_i < sha_length; byte_i++)
    {
        EXPECT_EQ(ufm_pit_pch_fw_hash[byte_i], exepected_pch_fw_hash[byte_i]);
        EXPECT_EQ(ufm_pit_bmc_fw_hash[byte_i], exepected_bmc_fw_hash[byte_i]);
    }
