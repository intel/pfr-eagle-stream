...
TEST_F(SMBusUtilsTest, test_apply_smbus_rules_from_bmc)
{
    ...
    PFM* active_pfm = get_active_pfm(SPI_FLASH_BMC);
    alt_u32_memcpy((alt_u32*) active_pfm, (alt_u32*) m_raw_pfm_x86, 256);

    // Whitelists from the SMBus rules are configured after the SMBus rule definitions are validated
    is_active_pfm_valid(active_pfm);
