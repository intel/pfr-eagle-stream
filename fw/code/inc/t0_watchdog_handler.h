...
#ifndef EAGLESTREAM_INC_T0_WATCHDOG_HANDLER_H_
#define EAGLESTREAM_INC_T0_WATCHDOG_HANDLER_H_
...
static void me_watchdog_timer_handler()
{
    ...
    if ((wdt_enable_status & WDT_ENABLE_ME_TIMER_MASK) && IORD(WDT_ME_TIMER_ADDR, 0))
    {
        ...
        else
        {
            ...
            {
                // ME Boot successfully
                if (!check_bit(U_GPI_1_ADDR, GPI_1_FM_ME_PFR_1))
                {
                ...
                // Clear the ME DONE detection flag
                // This ensure that check of ME boot status always at assertion edge of ME Boot Done only
                set_bit(U_GPO_1_ADDR, GPO_1_CLEAR_ME_DONE_FLAG);
                clear_bit(U_GPO_1_ADDR, GPO_1_CLEAR_ME_DONE_FLAG);
            }
        }
    }
}
...
/**
 * @brief Monitor if BMC clear the post seamless update bit. If so, clear the WDT timer and spi state
 *
 * @param update_src_spi_flash: Source spi flash
 *
 * @return none
 */
static void bmc_seamless_complete_actions(SPI_FLASH_TYPE_ENUM update_src_spi_flash)
{
    // Clear the BMC timer and seamless state
    IOWR(WDT_BMC_TIMER_ADDR, 0, 0);
    clear_spi_flash_state(update_src_spi_flash, SPI_FLASH_STATE_POST_SEAMLESS_UPDATE_MASK);
}
...
static void bmc_boot_complete_actions()
{
    ...
    // When BMC finish boot up, CPLD will start the timer to challenge BMC
...
static void acm_bios_watchdog_timer_handler()
{
    ...
    if ((wdt_enable_status & WDT_ENABLE_ACM_BIOS_TIMER_MASK) && IORD(WDT_ACM_BIOS_TIMER_ADDR, 0))
    {
        ...
        if (!(read_from_mailbox(MB_CONTROL_STATUS_READ) & MB_CMD_EMPTY_RD_FIFO_STATUS_MASK))
        {
            chkpt_cmd = read_from_mailbox(MB_BIOS_CHECKPOINT_READ_FIFO);
        }
#endif
        if (chkpt_cmd)
        {
...
#ifndef USE_SYSTEM_MOCK
                // Empty the checkpoint read fifo
                flush_checkpoint_mailbox_fifo();
#endif
            }
...
#endif /* EAGLESTREAM_INC_T0_WATCHDOG_HANDLER_H_ */
