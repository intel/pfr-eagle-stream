...
#ifndef EAGLESTREAM_INC_TMIN1_ROUTINES_H_
#define EAGLESTREAM_INC_TMIN1_ROUTINES_H_
...
/**
 * @brief This function handles the deep sleep logic, which is executed before releasing BMC/PCH from resets.
 *
 * Steps:
 * 1. Drive DSW PWROK to tristate (pulls up high)
 * 2. Wait 120ms
 * 3. Set SLP_SUS low
 */
static void deep_sleep_routine()
{
    ...
    // In this reference, it is needed to set some delay between DSW_PWROK and PFR_SLP_SUS to approx 4 seconds
    sleep_20ms(6);
...
static void tmin1_boot_pch()
{
    ...
    if (!(check_spi_flash_state(SPI_FLASH_PCH, SPI_FLASH_STATE_ALL_REGIONS_FAILED_AUTH_MASK) ||
          ...
          check_spi_flash_state(SPI_FLASH_PCH, SPI_FLASH_STATE_ATTESTATION_LOCKDOWN_RESET_MASK))
    {
        // Release SPI flash control to PCH
        release_spi_ctrl(SPI_FLASH_PCH);

        // Clear the internal ME DONE detection flag before release PCH from reset
...
static void tmin1_boot_bmc_and_pch()
{
    ...
    // Wait for 20ms between de-asserting SRST and RSMRST
    sleep_20ms(1);
...
static void perform_tmin1_operations_for_bmc()
{
    ...
    // Perform Attestation recovery if there was a failure and is set in the policy
...
#endif /* EAGLESTREAM_INC_TMIN1_ROUTINES_H_ */
