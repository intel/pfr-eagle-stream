...
#ifndef EAGLESTREAM_INC_T0_PROVISIONING_H_
#define EAGLESTREAM_INC_T0_PROVISIONING_H_
 ...
 * on the platform.
 *
 * NOTE: If only SPI Geometry is provisioned, ufm cmd will not be locked
 *
 * @param ufm_cmd a UFM provisioning command read from the mailbox register.
 */
static void mb_process_lock_ufm_cmd(alt_u32 ufm_cmd)
{
    if (ufm_cmd == MB_UFM_PROV_END)
    {
        ...
        else
        {
            ...
            {
                write_ufm_svn(UFM_SVN_POLICY_PCH, pch_svn);
            }
            alt_u32 bmc_svn = read_from_mailbox(MB_BMC_PFM_RECOVERY_SVN);
            if (bmc_svn <= UFM_MAX_SVN)
            {
                write_ufm_svn(UFM_SVN_POLICY_BMC, bmc_svn);
            }
        }
    }
}
...
static void mb_process_ufm_cmd()
{
    ...
    alt_u32* ufm_attest_pubkey_0 = get_ufm_ptr_with_offset(UFM_CPLD_PUBLIC_KEY_0);
#endif
    ...
    {
#ifdef USE_384_HASH
        write_mb_fifo_from_ufm((alt_u8*) ufm_data->root_key_hash_384, SHA384_LENGTH);
#else
        write_mb_fifo_from_ufm((alt_u8*) ufm_data->root_key_hash_256, SHA256_LENGTH);
#endif
    }
...
#if defined(ENABLE_ATTESTATION_WITH_ECDSA_384) || defined(ENABLE_ATTESTATION_WITH_ECDSA_256)
    else if (ufm_cmd == MB_UFM_PROV_RD_LAYER_0_PUBKEY)
    {
        // Read the public key l0 stored in the UFM
        // Write the key to the mb fifo
        // Read/write 96 bytes in total
    	write_mb_fifo_from_ufm((alt_u8*) ufm_attest_pubkey_0, UFM_CPLD_PUB_KEY_SIZE);
    }
#endif
...
#ifdef USE_384_HASH
            write_ufm_from_mb_fifo(ufm_data->root_key_hash_384, SHA384_LENGTH);
#else
            write_ufm_from_mb_fifo(ufm_data->root_key_hash_256, SHA256_LENGTH);
#endif
            }
        }
...
#if defined(ENABLE_ATTESTATION_WITH_ECDSA_384) || defined(ENABLE_ATTESTATION_WITH_ECDSA_256)
        else if (ufm_cmd == MB_UFM_PROV_ENABLE_ATTEST_CHALLENGE)
        {
            if (check_ufm_status(UFM_STATUS_ENABLE_CPLD_TO_CHALLENGE))
            {
                // Cpld enabled challenge status bit has been enabled. Must erase before provisioning again.
                mb_set_ufm_provision_status(MB_UFM_PROV_CMD_ERROR_MASK);
            }
            else
            {
                set_ufm_status(UFM_STATUS_ENABLE_CPLD_TO_CHALLENGE);
                mb_set_ufm_provision_status_2(MB_UFM_PROV_ATTESTATION_ENABLED);
            }
        }
        else if (ufm_cmd == MB_UFM_PROV_DISABLE_ATTEST_CHALLENGE)
        {
            if (check_ufm_status(UFM_STATUS_DISALLOW_CPLD_TO_CHALLENGE))
            {
                // Cpld disable challenge status bit has been enabled. Must erase before provisioning again.
                mb_set_ufm_provision_status(MB_UFM_PROV_CMD_ERROR_MASK);
            }
            else
            {
                set_ufm_status(UFM_STATUS_DISALLOW_CPLD_TO_CHALLENGE);
                mb_clear_ufm_provision_status_2(MB_UFM_PROV_ATTESTATION_ENABLED);
            }
        }
#endif
...
#endif /* EAGLESTREAM_INC_T0_PROVISIONING_H_ */
