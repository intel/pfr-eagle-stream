...
class QKY_READER :
    ...
    def __init__(self, filename, openssl, family, external_data, block0) :
    ...
    def get_key(self) :
        ...
        if len(self.keys) :
            ...
            common_util.assert_in_error(len(self.fuse_info), "Missing fuse info")
        else :
            common_util.assert_in_error(len(self.fuse_info) == 0, "There should not be any fuse info yet")
            ...
            for i in range(int(curve_info.size/4)) :
                self.fuse_info = "%s %08X" % (self.fuse_info, sha.get_dword(i * 4))
            self.fuse_info = self.fuse_info[1:]
            del sha
