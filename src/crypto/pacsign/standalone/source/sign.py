...
class SIGNER(object) :
    ...
    def insert_data(self, input_file, output_file, type) :
        ...
        reader = bitstream.get_family_reader(self.family, input_file, self.openssl, True, type, False)
    ...
    def sign(self, input_file, input_qky, private_pem, output_file, insert, type) :
    
        reader = bitstream.get_family_reader(self.family, input_file, self.openssl, insert, type, False)
    ...
    def fuse_info(self, input_qky, output_text) :
        ...
        file.write("Fuse: %s" % qky.fuse_info)
        file.close()
    ...
    def sign_bitstream(self, rbf, qky, private_key) :
        ...
        common_util.assert_in_error((len(qky.keys) - 1) >= self.database.MIN_CODE_SIGNING_KEY_ENTRIES, "Family %s must have at least %d Code Signing Key entry, but found %d" % (self.family, self.database.MIN_CODE_SIGNING_KEY_ENTRIES, len(qky.keys) - 1))
        common_util.assert_in_error((len(qky.keys) - 1) <= self.database.MAX_CODE_SIGNING_KEY_ENTRIES, "Family %s cannot have more than %d Code Signing Key entry, but found %d" % (self.family, self.database.MAX_CODE_SIGNING_KEY_ENTRIES, len(qky.keys) - 1))
