...
class CURVE_INFO :
    ...
    def __init__(self, enum, name, size, curve_magic_num, sha_magic_num, supported_families) :
...
CURVE_INFO_DATABASE = [
                        ...
                        CURVE_INFO(NID_X9_62_prime256v1,    "prime256v1",       32,   0xC7B88C74, 0xDE64437D, ["PAC_CARD"]),
                        CURVE_INFO(NID_secp384r1,           "secp384r1",        48,   0x08F07B47, 0xEA2A50E9, ["PAC_CARD"])
                    ]
...
def password_callback(buf, bufsiz, verify, cb_temp) :
    ...
    if cb_temp != None :
        for char in cb_temp :
            if char != 0 :
                comment += chr(char)
    common_util.assert_in_error(bufsiz >= 4, "Minimum password must be 4 characters, but only %d buffer size is being provided" % bufsiz)
    try :
        if verify :
            (password, error) = common_util.get_password(["Enter PEM passphrase (4 to %d characters per OpenSSL spec, Intel recommends minumum 13 characters): " % (bufsiz - 1), "Re-enter PEM passphrase (4 to %d characters per OpenSSL spec, Intel recommends minumum 13 characters): " % (bufsiz - 1)], 4, bufsiz - 1, comment)
        else :
            (password, error) = common_util.get_password(["Enter PEM passphrase (4 to %d characters per OpenSSL spec, Intel recommends minumum 13 characters): " % (bufsiz - 1)], 4, bufsiz - 1, comment)
...
class OPENSSL_SIGNATURE(Structure) :
    ...
    _fields_ = [("r", c_void_p),    # void *
                ("s", c_void_p)]    # void *
                
class OPENSSL_SHA256(Structure) :

    _fields_ = [("h", c_ulong * 8),
                ("N1", c_ulong),
                ("Nh", c_ulong),
                ("data", c_ulong * 16),
                ("num", c_uint),
...
class OPENSSL_SHA512_DATA(Union) :
    ...
    _fields_ = [("d", c_longlong * 16),
                ("p", c_char * 128)]
                
class OPENSSL_SHA512(Structure) :
    
    _anonymous_ = ("data",)
    _fields_ = [("h", c_longlong * 8),
                ("N1", c_longlong),
                ("Nh", c_longlong),
...
class openssl :
    ...
    def __init__(self) :
        ...
        self.lib = CDLL("%s" % dll)
        common_util.assert_in_error(self.lib != None, "Fail to load library %s" % dll)
        ...
        common_util.assert_in_error(len(version) == 5, "SSLeay_version() should have five information, only by found %d (%s) information" % (len(version), version))
        # Check the naming and version but ignore date, month, year
        common_util.assert_in_error(version[0] == "OpenSSL", "SSLeay_version() header information should be \"OpenSSL\", but found \"%s\"" % version[0])
        common_util.assert_in_error(version[1] == "1.0.2q", "SSLeay_version() version information should be \"1.0.2q\", but found \"%s\"" % version[1])
        ...
        # Write key to PEM
        self.lib.PEM_write_bio_ECPrivateKey.argtypes = [c_void_p, c_void_p, c_void_p, c_char_p, c_int, c_void_p, c_void_p]
        ...
        self.lib.PEM_read_bio_ECPrivateKey.argtypes = [c_void_p, c_void_p, c_void_p, c_char_p]
        ...
        self.lib.EC_KEY_set_public_key_affine_coordinates.argtypes = [c_void_p, c_void_p, c_void_p]
        ...
        self.lib.EC_POINT_get_affine_coordinates_GFp.argtypes = [c_void_p, c_void_p, c_void_p, c_void_p, c_void_p]
        ...
        # Verify
        self.lib.ECDSA_do_verify.argtypes = [c_char_p, c_int, POINTER(OPENSSL_SIGNATURE), c_void_p]
    ...
    def generate_group(self, nid) :
        ...
        self.lib.EC_GROUP_set_point_conversion_form(group, POINT_CONVERSION_UNCOMPRESSED)    
        return group
        
    def generate_key(self, group) :
    
        key = self.lib.EC_KEY_new()
        common_util.assert_in_error(self.lib.EC_KEY_set_group(key, group) != 0, "generate_key() failed to EC_KEY_set_group()")
        common_util.assert_in_error(self.lib.EC_KEY_generate_key(key) != 0, "generate_key() failed to EC_KEY_generate_key()")
        return key
        
    def generate_private_pem(self, pem, group, key, encrypt) :
    
        bio = self.lib.BIO_new(self.lib.BIO_s_file())
        status = self.lib.BIO_ctrl(bio, BIO_C_SET_FILENAME, BIO_CLOSE | BIO_FP_WRITE, pem.encode('utf-8'))
        common_util.assert_in_error(status > 0, "Fail to open file %s for BIO write" % pem)
        ...
        if encrypt == None or encrypt == False:
            status = self.lib.PEM_write_bio_ECPrivateKey(bio, key, None, None, 0, None, None)
        else :
            enc = self.lib.EVP_get_cipherbyname("aes256".encode('utf-8'))
            status = self.lib.PEM_write_bio_ECPrivateKey(bio, key, enc, None, 0, callback, ("Writing %s" % pem).encode('utf-8'))
        common_util.assert_in_error(status > 0, "Fail to write EC Private Key to PEM BIO")
    ...
    def generate_public_pem(self, pem, key) :
        ...
        common_util.assert_in_error(status > 0, "Fail to write EC Publick Key to PEM BIO")
        self.lib.BIO_free_all(bio)
        
    def read_private_key(self, private_pem) :
    
        bio = self.lib.BIO_new(self.lib.BIO_s_file())
        status = self.lib.BIO_ctrl(bio, BIO_C_SET_FILENAME, BIO_CLOSE | BIO_FP_READ, private_pem.encode('utf-8'))
        common_util.assert_in_error(status > 0, "Fail to read file %s for BIO read" % private_pem)
        key = self.lib.PEM_read_bio_ECPrivateKey(bio, None, callback, ("Reading %s" % private_pem).encode('utf-8'))
        common_util.assert_in_error(key != None, "Fail to read EC Private Key from PEM BIO %s" % private_pem)
        self.lib.BIO_free_all(bio)
        return key
    ...
    def read_public_key(self, public_pem) :
        ...
        bio = self.lib.BIO_new(self.lib.BIO_s_file())
        status = self.lib.BIO_ctrl(bio, BIO_C_SET_FILENAME, BIO_CLOSE | BIO_FP_READ, public_pem.encode('utf-8'))
        common_util.assert_in_error(status > 0, "Fail to read file %s for BIO read" % public_pem)
        pub_key = self.lib.PEM_read_bio_PUBKEY(bio, None, None, None)
        common_util.assert_in_error(pub_key != None, "Fail to read EC Public Key from PEM BIO %s" % public_pem)
        key = self.lib.EVP_PKEY_get1_EC_KEY(pub_key)
        common_util.assert_in_error(key != None, "Fail to get EC Key from public key")
    ...
    def generate_ec_key_using_xy_and_curve_info(self, xy, curve_info) :
        ...
        common_util.assert_in_error(len(xy) == (curve_info.size * 2), "%s xy size should be %d Bytes, but found %d Bytes" % (curve_info.name, curve_info.size * 2, len(xy)))
        ...
        common_util.assert_in_error(status > 0, "Fail to set public key affine coordinates")
    ...
    def get_bignums_from_byte_array(self, byte_array) :
        ...
        common_util.assert_in_error(len(byte_array) == 64 or len(byte_array) == 96, "get_bignums_from_byte_array() expects data size to be 64 or 96 Bytes, but found %d Bytes" % len(byte_array))        
        byte_array_size = int(len(byte_array)/2)
        return [self.get_bignum_from_byte_array(byte_array[:byte_array_size]), self.get_bignum_from_byte_array(byte_array[byte_array_size:])]
    ...
    def get_byte_array_sha(self, type, bytes) :
        ...
        common_util.assert_in_error(type == 32 or type == 48 or type == 64, "Expect SHA supported type size is 32, 48 or 64 Bytes, but found %d Bytes" % type)
        common_util.assert_in_error(len(bytes) > 0 and (len(bytes) % 128) == 0, "Data to be hashed must have size greater than zero and multiple of 128 Bytes, but found %d Bytes" % len(bytes))
        cpointer = common_util.CHAR_POINTER(len(bytes))
        sha = common_util.CHAR_POINTER(type)
        cpointer.assign_data(bytes)
        if type == 32 :
            self.get_sha256(cpointer.data, len(cpointer.data), sha)
        elif type == 48 :    
            self.get_sha384(cpointer.data, len(cpointer.data), sha)
        else :
            self.get_sha512(cpointer.data, len(cpointer.data), sha)
        del cpointer
        return sha
