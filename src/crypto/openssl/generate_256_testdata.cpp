...
EC_KEY* generate_key()
{
    ...
    cout << "\t\t256'h" << privkey_hex_str << endl;
    ...
    cout << "Pubkey:" << endl;
    cout << "\tCX : 256'h";
    for (int i = 2; i < (2 + 64); i++) // 1 byte 0x40, 256/8 bytes for X coordinate, 256/8 bytes for Y coordinate
    {
...
void create_signature(const std::string& digest, EC_KEY* eckey)
{
    ...
    cout << "Hash:" << endl << "\tE :  256'h" << BN_bn2hex(hash_bn) << endl;
    ...
    cout << "Signature:"<< endl;
    cout << "\tR :  256'h" << signature_r_hex_str << endl;
    cout << "\tS :  256'h" << signature_s_hex_str << endl;
...
int main()
{
    ...
    printf("ECDSA256/SHA256 Test Generator\n");
    ...
    for (int block_index = 0; block_index < patterns_per_block.size(); block_index++) {
        ...
        {
            std::vector<unsigned char> test_data_complete(BLOCK_SIZE_BYTES*num_blocks);
            ...
            for (int block = 0; block < num_blocks; block++)
            {
                ...
                {
                    test_data_complete[block*BLOCK_SIZE_BYTES + i] = test_data[i];
                }
            ...
            cout << "Total Test data : " << (num_blocks * BLOCK_SIZE_BYTES * 8) << "'h" << print_hex_string(test_data_complete) << endl;

            // Calculate the SHA on the data and print it
            const std::string digest = calculate_sha(mdctx);
            cout << "Digest : 256'h" << digest << endl;
